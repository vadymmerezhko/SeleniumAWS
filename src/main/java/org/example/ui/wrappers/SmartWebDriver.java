...
public class SmartWebDriver implements WebDriver, JavascriptExecutor, TakesScreenshot {
    ...
    private final WebSynchronizer synchro;
    ...
    public SmartWebDriver(WebDriver driver) {
        this.driver = driver;
        synchro = new WebSynchronizer(driver);
    ...
    public List<WebElement> findElements(By by) {
        ...
        List<WebElement> smartElements = elements.stream().map(element ->
                new SmartWebElement(element, null, by, driver))
    ...
    public WebElement findElement(By by) {
        ...
        while ((System.currentTimeMillis() - startMilliseconds) < waitTimeoutMilliseconds) {
            ...
            if (size == 1) {
                ...
                return new SmartWebElement(element, null, by, driver);
            }
            TimerUtils.waitMilliSeconds(WAIT_ELEMENT_DELAY_MILLISECONDS);
            waitForPageLoad();
        }
        element = new SmartWebElement(driver.findElement(by), null, by, driver);
        log.debug("Web element {} is found by selector {}.", element, by);
        return element;
    }
