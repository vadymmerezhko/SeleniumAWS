...
public final class DockerUtils {
    ...
    public static String stopAllContainers() {
        ...
        String output = SystemUtils.runCommandLine("docker ps -a -q");
        int size = output.length() / 12;

        for (int i = 0; i < size; i++) {
            String id = output.substring(i * 12, (i + 1)  *12);
            result.append(SystemUtils.runCommandLine(String.format("docker stop %s", id))).append("\n");
        }
    ...
    public static String removeAllContainers() {
        return SystemUtils.runCommandLine("docker container prune -f");
    }
    ...
    public static String runSeleniumHub() {
        return SystemUtils.runCommandLine(
    ...
    public static String runSeleniumNode(String browserName, String browserVersion) {
        ...
        return SystemUtils.runCommandLine(String.format(
